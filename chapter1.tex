...
\begin{definition}
  ...
  \begin{enumerate}
    \item\label{intro-relations-bds-1} $R(x):= \{ y\in X: (x, y) \in R \}$, and so, $R$ can be viewed as a mapping $x \mapsto R(x)$;
...
\begin{proposition}
  ...
  \[
    \begin{tikzcd}[row sep = small]
      &\{ R \in P(X^2): \text{equivalent relation} \}
...
Based on this, we obtain the following propositions.
\begin{proposition}
  \label{prop:intro-ideal-props}(...)
  \begin{enumerate}
