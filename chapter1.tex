...
\begin{definition}
  \label{def:intro-basics-bds}\hfill\par 
  \begin{enumerate}
    \item\label{intro-basics-bd-1} $S$ is a null semigroup if $\forall x, y \in S (xy = 0)$;
    \item\label{intro-basics-bd-2} $S$ is a left zero semigroup if $\forall x, y \in S (xy = x)$, the definition for right zero semigroup is obvious;
    ...
    \item\label{intro-basics-bd-6} a semigroup $S$ is a rectangular band if $\forall a, b \in S (aba = a)$;
    \item\label{intro-basics-bd-7} $\gen{\{ a \}}{\mathrm{smg}}$ is called a \textit{monogenic semigroup}.
  \end{enumerate}
\end{definition}
...
% \begin{proposition}
%   Suppose $S$ is a semigroup, the propositions listed below are equivalent.
%   \label{prop: smgisgrp}
%   \begin{enumerate}
...
% \end{proposition}
% \begin{proof}
%   It is easy to demonstrate that \ref{intro-basics-smggrp-1} $\Rightarrow$ \ref{intro-basics-smggrp-3} and \ref{intro-basics-smggrp-2} $\Leftrightarrow$ \ref{intro-basics-smggrp-3}. So we proceed to prove \ref{intro-basics-smggrp-3} $\Rightarrow$ \ref{intro-basics-smggrp-1}, and it is suffices to show that $S$ has the unique identity, and that for any element, its inverse exists and is unique. Let $ax = ya = a$, and it follows that
%   \[
%     x = ax_1 = ay_1a = yay_1a = yx = ax_2ax = ax_2a = y_2a = y.
%   \]
%   Thus, we may conclude that every element $a$ in $S$ has an identity $\epsilon_a$ such that $\epsilon_a a = a \epsilon_a = a$. Now, the issue lies in proving $\epsilon_a = \epsilon_b$ for any $a, b$ in $S$, and the method is analogous:
%   \[
...
% \begin{theorem}
%   Suppose $S$ is a semigroup, $X = S^1$, then there exists a faithful representation 
%   \[
%     \varphi: S \to \End(X).
%   \]
% \end{theorem}
...
% \begin{theorem}
%   Suppose $S$ is a semigroup, the following propositions are equivalent.
%   \begin{itemize}
%     \item $S$ is a rectangular band \ref{intro-basics-bd-6},
...
% To study the monogenic subsemigroup, we introduce the following concepts. Suppose $a$ is an element in $S$, and its order is finite if not specified.
% \begin{nabls}{MSD}
%   \nabitem{intro-monos-msd-0} $\gen{a}{}:= \gen{\{a\}}{\mathrm{smg}}$,
%   \nabitem{intro-monos-msd-1} $\operatorname{ord}(a) := |\langle a \rangle|$,
%   \nabitem{intro-monos-msd-2} $\operatorname{idx}(a) := \min \left\{ m \in \Z_{\geq 1}: \exists n \in \Z_{\geq 1} (a^m = a^n \land m \neq n) \right\}$,
%   \nabitem{intro-monos-msd-3} $\operatorname{prd}(a) := \min \left\{ r \in \Z_{\geq 1}: a^{m+r} = a^m \right\}$,
%   \nabitem{intro-monos-msd-4} a semigroup is called \textit{periodic} if all its elements are of finite order.
% \end{nabls}
% Let $m = \operatorname{idx}(a)$, $r = \operatorname{prd}$, clearly, $a, a^2, \cdots, a^{m+r-1}$ are mutually different, and $\langle a \rangle = \{ a, \cdots, a^{m+r-1} \}$. 
...
%   \[
%     \gen{a}{} \simeq \gen{b}{} \Leftrightarrow (\operatorname{idx}(a), \operatorname{prd}(a)) = (\operatorname{idx}(b), \operatorname{prd}(b)).
%   \]
% \end{proposition}
% \begin{proof}
%   Suppose $\operatorname{idx}(a) = \operatorname{idx}(b) = m$ and $\operatorname{prd}(a) = \operatorname{ord}(b) = r$, the mapping defined below is an isomorphism.
%   \[
%     \begin{tikzcd}[row sep = small]
%       &\{ a, \cdots, a^{m+r-1} \}
%         \arrow[r, "\sim"]
...
%   For the reverse, assume $\gen{a}{} \xrightarrow[\phi]{\sim} \gen{b}{}$, where $\phi$ maps $a$ to $b^{\xi}$, it is straightforward to verify that $\gen{b^{\xi}}{} = \gen{b}{}$ and that $\operatorname{idx}(a) = \operatorname{idx}(b^{\xi})$, $\operatorname{prd}(a) = \operatorname{prd}(b^{\xi})$. If $\xi = 1$, the proof is over. On the other hand, if $\xi > 1$, then there exists $\mu \geq 1$ such that $b^{\xi\mu} = b$, thus, $\operatorname{idx}(b) = 1$, which implies $\gen{b}{}$ is a cyclic group. Hence, $\gen{a}{}$ is also a cyclic group with the generator $\phi^{-1}(b) = a^{\zeta}$. Since $a$ is a generator, similarly, there exists an integer $\nu$ that makes $a^{\zeta\nu} = a$, and it follows that $\operatorname{idx}(a) = 1$. Furthermore, observe that $\operatorname{prd}(a) = \vert\gen{a}{}\vert = \vert\gen{b}{}\vert = \operatorname{prd}(b)$.
% \end{proof}
...
% \begin{proposition}
%   For any pair $(m, r) \in \Z_{\geq 1}^2$, there exists a semigroup $S$ containing an element with $\operatorname{idx}$ of $m$ and $\operatorname{prd}$ of $r$. 
% \end{proposition}
% \begin{proof}
...
% \]
% where $R_i$ is the element in $P(X^2)$, forms a semigroup. To see this, it is suffices to verify $\circ$ is associative, which is obvious. Some brief definitions are listed as follows.
% \begin{nabls}{RD}
...
% We then introduce the definitions of partial orders and equivalent relations from this perspective. A partial order is a relation satisfies the following conditions,
% % \begin{nabls}{POD}
% %   \nabitem{intro-rela-pod-1} (reflective) $\Delta_X \subset R$,
% %   \nabitem{intro-rela-pod-2} (anti-symmetric) $R\cap R^{\op} = \Delta_X$,
% %   \nabitem{intro-rela-pod-3} (transitive) $R^2 \subset R$. 
% % \end{nabls}
% \begin{itemize}
...
% \end{itemize}
% On another hand, $R$ is an equivalence relation is the conditions below are all hold,
% % \begin{nabls}{EQD}
% %   \nabitem{intro-rela-eqd-1} (reflective) $\Delta_X \subset R$,
% %   \nabitem{intro-rela-eqd-2} (symmetric) $R^{\op} \subset R$,
% %   \nabitem{intro-rela-eqd-3} (transitive) $R^2 \subset R$. 
% % \end{nabls}
% \begin{itemize}
%   \item (reflective) $\Delta_X \subset R$,
...
% \begin{proposition}
%   Given a set $X$, a partition $\calA$ is a family of disjoint subsets of $X$ satisfying $\bigsqcup \calA = X$. Then, there exists a bijection
%   \[
%     \begin{tikzcd}[row sep = small]
...
% \end{equation}
% Thus, both symbol $\land$ and $\lor$ on lattice $\Eqv(S)$ and $\Eqv(S)$ represent the same operations of sets. Here are some conclusions for supplementation, which can be found in \cite[p.28]{John-FundSmg}. Suppose $R_1, R_2$ are equivalences, then
% \begin{nabls}{CP}
