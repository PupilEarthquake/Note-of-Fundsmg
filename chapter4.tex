...
\[
  \bmrm{F} := \left\{ T \subset S: F_0 \subset T \land \left( \forall \omega \in \Omega, \forall \bm{u} \in T^{\ar \omega} (H_{\omega}(\bm{u}) \in T) \right) \right\}.
\]
...
\begin{definition}
  Given a set $X$, the $\calV$-\textit{free algebra} $\bmrm{F}_{\calV}(X)$ is an object of $\calV$ satisfies for any $L \in \Ob{\calV}$ and any mapping $f: X \to L$, there exist a unique morphism $\psi$ such that the following diagram commutes:
  \[
    \begin{tikzcd}
      X
        \arrow[r, hook, "\iota"]
...
\begin{definition}
  ...
  \begin{itemize}
    ...
    \item $c_{\calV}(X) := \bigcap_{(f, S) \in \calO} \fker(f, X, S)$, where $\calO = \{ (f, S) \in \Mor(\cate{Set}) \times \Ob(\calV): f \in \Hom_{\cate{Set}}(X, S) \}$;
    \item for $u, v$ in $\bmrm{F}(X)$, $\ceq(X, u, v):= \left\{ S \in \Ob(\calV): (u, v) \in \bigcap_{f \in \Hom_{\cate{Set}}(X, S)} \fker(f, X, S) \right\}$;
    \item given a binary relation $R \subset \bmrm{F}(X)^2$, the equational class $\ceq(X, R):= \bigcap_{(u,v)\in R}\ceq(X, u, v)$.
  \end{itemize}
\end{definition}
...
\[
  \begin{tikzcd}[row sep = small]
    \bmrm{F}(X)
      \arrow[r]
    & \prod_{(f, S) \in \calO} \bmrm{F}(X) / \fker(f, X, S) \\
    u
      \arrow[r, mapsto]
    &\left[ (f, S) \mapsto  \fker(f, X, S)(u) \right]
  \end{tikzcd}
\]
where $\calO = \{ (f, S) \in \Mor(\cate{Set}) \times \Ob(\calV): f \in \Hom_{\cate{Set}}(X, S) \}$. Then, 
\[
  \Theta(u) = \Theta(v) \Leftrightarrow c_{\calV}(X)(u) = c_{\calV}(X)(v),
\]
and it follows that 
\[
  \bmrm{F}(X) /  c_{\calV}(X) \simeq \im \Theta \subset \prod_{(f, S) \in \calO} \bmrm{F}(X) / \fker(f, X, S) \in \Ob \calV,
\]
...
\begin{theorem}
  As a set, $\calV$ is a variety if and on if it is an equational class $\ceq(X, R)$. 
\end{theorem}
\begin{proof}
  Given a set $X$ and a relation $R \subset \bmrm{F}(X)^2$, we assert that $\ceq(X, R)$ is a variety. The proof proceeds in three parts:
  \begin{enumerate}
    \item for any $ S\in \ceq(X, R)$ and subalgebra  $T \subset S$, $T \in \ceq(X, R)$, since for any $f \to T$ the following diagram commutes, and so $\forall (u,v) \in R (\bar f (u) = \bar f (v))$.
    \[
      \begin{tikzcd}
        X
          \arrow[r, hook]
      ...
      \end{tikzcd}
    \]
    \item For any $ S\in \ceq(X, R)$ and the epimorphism  $\pi: S \to T$, $T \in \ceq(X, R)$. For any $f: S \to T$, let $g \in \prod_{x \in X}\pi^{-1}f(x)$, then $X \xrightarrow{g} S \xrightarrow{\pi} T = f$. By the commutativity of the diagram below, $\bar f(u) = \bar f(v)$ for any $(u, v) \in R$.
    \[
      \begin{tikzcd}
        X
          \arrow[r, hook]
      ...
      \end{tikzcd}
    \] 
    \item Suppose $I$ is small and $S_i \in \ceq(X, R)$, then so does $\prod_{i \in I}S_i$. This is from the following commutative diagram, in which $p_i$ is the projection, which implies that 
    \[
  ...
  Conversely, given a variety $\calV$ and a countably finite set $X$, we assert that $\calV = \ceq(X, c_{\calV}(X))$. The containment $\calV \subset \ceq(X, c_{\calV}(X))$ is easy to verify. For the reverse, let $S \in \ceq(X, c_{\calV}(X))$. Choose an infinite set $Y$ that $|Y| \geq |X|$ and $|Y| \geq |S|$. We shall show that $S$ is the image of morphism $[\bmrm{F}_{\calV}(Y) \twoheadrightarrow S]$, and so $S \in \calV$. 

  Suppose $(u, v) \in c_{\calV}(Y)$, then $u,v \in \bmrm{F}(Y)$, each of them can be viewed as a series of finite length that every coordinate belongs to set $Y \sqcap \Omega \sqcup \{(, )\}$ respectively, and so could it be viewed as a function which has a finite domain contained in $\Z_{\geq 1}$ and a finite image contained in $Y \sqcap \Omega \sqcup \{(, )\}$. Let $Y_0 = (\im u \cup \im v) \cap Y$, clearly it is a finite set. Let $X_0 \subset X$, and we may find a function $\xi_0: X \hookrightarrow Y$ that satisfies $\xi_0(X_0) = Y_0$.
\end{proof}