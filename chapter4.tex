...
% \begin{proposition}
%   \label{prop:crgrp-crgrp-eqp}
%   Let $S$ be a semigroup, the following propositions are equivalent.
%   \begin{enumerate}
%     \item\label{crgrp-crgrp-eqp1-1} $S$ is c.r.;
%     \item\label{crgrp-crgrp-eqp1-2} every element is $S$ lies in a subgroup of $S$;
%     \item\label{crgrp-crgrp-eqp1-3} every $\calH$-class is a group.
%   \end{enumerate}
% \end{proposition}
% \begin{proof}
%   \ref{crgrp-crgrp-eqp1-1} $\Rightarrow$ \ref{crgrp-crgrp-eqp1-2} Given a c.r.s $(S, \cdot, (-)')$, for any $a \in S$, let $e = a'a = aa'$. Then by \ref{crgrp-crgrp-crp1-1}, $a$ belongs to a group $\calH(e)$.

%   \ref{crgrp-crgrp-eqp1-2} $\Rightarrow$ \ref{crgrp-crgrp-eqp1-3} Given a semigroup $(S, \cdot)$ that each element $a$ lies in a subgroup $G_a$ of $S$, define the mapping $(-)'$ that sends $a$ to its inverse in $G_a$. Then, from $aa' = a'a =: e_a$ and $e_a a = a e_a = a$, it follows that $a \calH e_a$, where $\calH(e_a)$ is a group, for $e_a$ is an idempotent.

%   \ref{crgrp-crgrp-eqp1-3} $\Rightarrow$ \ref{crgrp-crgrp-eqp1-1} Suppose $(S, \cdot)$ is a semigroup that every $\calH$-calss is a group. It's easy to verify that the mapping, $(-)'$, sending $a$ to its inverse $a' \in \calH(a)$, together with the presupposed structure on $S$, forms a c.r.s.
% \end{proof}
...
%   \begin{enumerate}
%     \item\label{crgrp-crgrp-eqp2-1} $S$ is c.s.
%     \item\label{crgrp-crgrp-eqp2-2} $S$ is c.r, and for all $x, y \in S$, $xx' = (xyx)(xyx)'$;
%     \item\label{crgrp-crgrp-eqp2-3} $S$ is c.r and simple.
%   \end{enumerate}
% \end{proposition}
% \begin{proposition}
%   \ref{crgrp-crgrp-eqp2-1} $\Rightarrow$ \ref{crgrp-crgrp-eqp2-2} By \ref{spgrp-css-cssp7} of Proposition \ref{spgrp-css-cssp}, every $\calH$-class of $S$ is a group, so we let $(-)'$ be the mapping that sending $a$ to its inverse within $\calH(a)$. Let $x, y \in S$, by \ref{spgrp-css-cssp6} of Proposition \ref{spgrp-css-cssp}, we obtain that $xy \in \calR(x) \cap \calL(y)$ and that $xyx \in \calR(xy) \cap \calL(x)$, in which $\calR(xy) = \calR(x)$. Thus, $xyx \in \calH(x)$. Hence, $xx' = (xyx)(xyx)'$.

%   \ref{crgrp-crgrp-eqp2-2} $\Rightarrow$ \ref{crgrp-crgrp-eqp2-3} Suppose $(S, \cdot, (-)')$ is a c.r.s and $a, b \in S$. Then 
%   \[
%     a = aa'a = a\cdot b \cdot a(aba)'a,
%   \]
%   and so $\calJ(a) \leq \calJ(b)$. By interchanging the role of $a$ and $b$, we may equally show that $\calJ(b) \leq \calJ(a)$. Thus, $\calJ(a) = \calJ(b)$ for any $a , b$ in $S$, and so $\calJ = S^2$, which implies $S$ is simple by \ref{spgrp-spgrp-sgeqps-1}.

%   \ref{crgrp-crgrp-eqp2-3} $\Rightarrow$ \ref{crgrp-crgrp-eqp2-1} This is a direct conclusion of Proposition \ref{spgrp-css-spps}.
% \end{proposition}
...
% \begin{proposition}
%   \label{prop:crgrp-crgrp-idmispri}
%   Suppose $(S, \cdot, (-)')$ is simple and c.r, then every idempotent of $S$ is primitive.
% \end{proposition}
% \begin{proof}
...
%   \] 
%   On the other hand, $S$ is c.r, so $x$ lies in a group $\calH$-class, in which there exists an identity $e_x$ satisfies $e_x x = x e_x = x$ and $x'x = xx' = e_x$. Observe that
%   \[
...
% We Stipulate that $\cate{Css}$ denotes the category of all c.s.s., which is a full subcategory of the semigroup category $\cate{Smg}$. A partially ordered set $(I, \leq)$ can be made into a category, where for any elements $i, j$ of $I$,
% \[
%   i \to j \Leftrightarrow j \leq i.
% \]
% Similarly, a semilattice can also be made into a category in the same manner.
...
%   To show that $A$ is c.r., observe that any $a \in A$ lies in a group $\calH(a) \subset \calJ(p)$, thus by Proposition \ref{prop:crgrp-crgrp-eqp}, it is a c.r.s.
